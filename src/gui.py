...
def ui_info():
    ...
    ui_spacer(1)
    st.markdown(
        "Source code can be found [here](https://github.com/mobarski/ask-my-pdf)."
    )
...
def ui_api_key():
    def on_change():
        ...
        ss["storage"] = storage.get_storage(data_dict=ss["data_dict"])
        ss["debug"]["storage.folder"] = ss["storage"].folder
        ss["debug"]["storage.class"] = ss["storage"].__class__.__name__
        ss["user"] = ss[
            "storage"
        ].folder  # TODO: refactor user 'calculation' from get_storage
    ...
    ss["api_key"] = os.environ["OPENAI_API_KEY"]
    on_change()
...
def index_pdf_file():
    if ss["pdf_file"]:
        ...
        index = model.index_file(
            ss["pdf_file"],
            fix_text=ss["fix_text"],
            frag_size=ss["frag_size"],
            pg=ss["pg_index"],
        )
        usage = index["usage"]
        ss["stats"].incr(
            "usage:v1:{date}:{user}", {"index:" + k: v for k, v in usage.items()}
        )
        ss["stats"].incr(
            "hourly:v1:{date}", {"index:" + k + ":{hour}": v for k, v in usage.items()}
        )
...
def ui_fragments():
    ...
    st.number_input(
        "fragments before", 0, 3, 1, key="n_frag_before"
    )  # TODO: pass to model
    st.number_input(
        "fragments after", 0, 3, 1, key="n_frag_after"
    )  # TODO: pass to model
...
def b_ask():
    ...
    if st.button("get answer", disabled=disabled, type="primary"):
        ...
        usage["cnt"] = 1
        ss["stats"].incr(
            "usage:v1:{date}:{user}", {"ask:" + k: v for k, v in usage.items()}
        )
        ss["stats"].incr(
            "hourly:v1:{date}", {"ask:" + k + ":{hour}": v for k, v in usage.items()}
        )
        ss["debug"]["stats"] = ss["stats"].get("usage:v1:{date}:{user}")
