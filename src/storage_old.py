...
class Storage:
    ...
    def __init__(self, secret_key):
        k = secret_key.encode()
        self.folder = hashlib.blake2s(
            k, salt=SALT, person=b"folder", digest_size=8
        ).hexdigest()
        self.passwd = hashlib.blake2s(
            k, salt=SALT, person=b"passwd", digest_size=32
        ).hexdigest()
...
class S3Storage(Storage):
    ...
    def _list(self):
        resp = self.s3.list_objects(Bucket=self.bucket, Prefix=self.get_key(""))
    ...
    def _delete(self, name):
        self.s3.delete_object(Bucket=self.bucket, Key=self.get_key(name))
